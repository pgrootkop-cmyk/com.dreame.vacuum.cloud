...
**The "Start vacuuming" ("Begin met stofzuigen") flow card doesn't sweep — it just starts the last cleaning task. Why?**

That card is generated by Homey itself from the vacuum device class — the app cannot rename it. Despite the wording, it is the generic "start" command: it starts (or resumes) a full clean with the currently selected cleaning mode, suction and water settings — the same as pressing play in the Dreame app. To explicitly vacuum without mopping, use **"Set cleaning mode to Sweeping"** before it, or use the room/zone cleaning cards which have their own cleaning-mode option.

**Sweeping and Sweeping & Mopping are swapped on my vacuum. What can I do?**

Different Dreame models (and firmwares) use opposite values for these two modes, and the cloud protocol is undocumented. The app detects the right mapping from the dock type, but if your vacuum still behaves inverted, go to the **device settings** (Devices → your vacuum → gear icon → Advanced settings) and change **"Sweeping / Sweeping & Mopping mapping"** from Auto to **Normal** or **Swapped**.
